...
class EnhancedAlienDecoder:
    def __init__(self):
        # Сначала инициализируем словарь
        self.dictionary = self._load_comprehensive_dictionary()
        
        # Затем остальные компоненты, которые зависят от словаря
    ...
    def _load_comprehensive_dictionary(self) -> Dict[str, str]:
        ...
        # Попытка импорта из модуля enhanced_dictionary
        try:
            from enhanced_dictionary import EXTENDED_DICTIONARY
            dictionary.update(EXTENDED_DICTIONARY)
        except ImportError:
            print("Предупреждение: модуль enhanced_dictionary не найден")
