...
class TatoebaDataset:
    ...
    def load_parallel_corpus(self, source_lang='eng', target_lang='rus', max_samples=None, progress_callback=None):
        ...
        # Если max_samples не указан, используем все связи
            if max_samples is None:
                total_links = len(links_df)
            else:
                total_links = min(len(links_df), max_samples * 3)
        
            processed_count = 0
            for idx, (_, row) in enumerate(links_df.iterrows()):
            # Если указан max_samples и достигли лимита - прерываем
                if max_samples is not None and len(parallel_pairs) >= max_samples:
                    break
                ...
                if progress_callback and processed_count % 1000 == 0:
                    progress = 70 + (processed_count / total_links) * 25
...
class ProgressWindow:
    ...
    def update(self, value, text=""):
        """Обновление прогресса - исправленная версия"""
        ...
        try:
            ...
            # Используем update_idletasks вместо update для избежания рекурсии
            self.window.update_idletasks()
        finally:
            self._update_in_progress = False
...
class ImprovedEnhancedTranslator:
    ...
    def detect_language_pair(self, text, target_lang='ru'):
        """Определение исходного языка с обработкой специальных случаев"""
        # Сначала проверяем на специальные языки
        special_lang = self.detect_special_language(text)
    
        if special_lang == "special_encrypted":
            # Для шифрованных текстов используем английскую модель как базовую
    ...
    # Стандартная обработка
        ...
        source_code = lang_map.get(special_lang, 'en')
    ...
    def download_tatoeba_corpus(self, source_lang='eng', target_lang='rus', progress_callback=None):
        ...
        # Передаем None для загрузки всех доступных предложений
        return self.tatoeba.load_parallel_corpus(source_lang, target_lang, max_samples=None, progress_callback=progress_callback)
...
class EnhancedTranslationGUI:
    ...
    def detect_special_language(self, text):
        """
        Определение специальных и искусственных языков
        """
        # Проверка на предоставленный пример языка
        characteristic_patterns = [
            r'\b[aä][nṅ]s\b', r'\b[äa]ig\b', r'\b[aä]ġ\b', 
            r'\b[dḋ]y\b', r'\b[ṅn]ytwnyḋ\b', r'\b[äa]slyṅ\b'
        ]
    
        pattern_count = sum(1 for pattern in characteristic_patterns 
                           if re.search(pattern, text.lower()))
    
        if pattern_count >= 3:
            return "special_encrypted"
    ...
    def setup_tatoeba_tab(self, parent):
        ...
        self.corpus_tree = ttk.Treeview(data_frame, columns=columns, show='headings', height=15)
        ...
        self.tree_context_menu.add_command(label="Использовать для перевода", command=self.use_for_translation)
        self.corpus_tree.bind("<Button-3>", self.show_tree_context_menu)
        
        # Пагинация
        pagination_frame = ttk.Frame(data_frame)
        pagination_frame.pack(side='bottom', fill='x', pady=5)
        
        ttk.Button(pagination_frame, text="⏪ Первая", 
                  command=lambda: self.show_page(0)).pack(side='left', padx=2)
        ttk.Button(pagination_frame, text="◀️ Назад", 
                  command=self.previous_page).pack(side='left', padx=2)
        
        self.page_label = ttk.Label(pagination_frame, text="Страница 1 из 1")
        self.page_label.pack(side='left', padx=10)
        
        ttk.Button(pagination_frame, text="Вперед ▶️", 
                  command=self.next_page).pack(side='left', padx=2)
        ttk.Button(pagination_frame, text="Последняя ⏩", 
                  command=self.last_page).pack(side='left', padx=2)
        
        # Настройки отображения
        display_frame = ttk.Frame(pagination_frame)
        display_frame.pack(side='right', padx=10)
        
        ttk.Label(display_frame, text="Записей на странице:").pack(side='left')
        self.page_size_var = tk.StringVar(value="50")
        page_size_combo = ttk.Combobox(display_frame, textvariable=self.page_size_var, 
        ...
        # Статистика - ИСПРАВЛЕНИЕ: создаем как атрибут класса
    ...
    def use_for_translation(self):
        """Использовать выбранный текст для перевода"""
        item = self.corpus_tree.selection()[0]
        values = self.corpus_tree.item(item, 'values')
        self.input_text.delete("1.0", tk.END)
        self.input_text.insert("1.0", values[0])
        # Переключаемся на вкладку перевода
...
to provide high-quality translations."""
    ...
    # Определяем язык перед переводом
        try:
            detected_lang = self.translator.detect_special_language(text)
            self.status_label.config(text=f"Определен язык: {detected_lang}")
        except Exception as e:
            self.status_label.config(text=f"Ошибка определения языка: {str(e)}")
    ...
    def download_tatoeba(self):
        ...
        result = messagebox.askyesno(
            "Предупреждение", 
            "Загрузка ВСЕХ доступных предложений может занять много времени и памяти.\n\n"
            "Продолжить?"
        )
    ...
    def _download_tatoeba_thread(self, source_lang, target_lang):
        ...
        try:
            last_update = 0
            update_interval = 0.1  # Минимальный интервал между обновлениями (секунды)
            ...
            def update_progress(value, text):
                ...
                if current_time - last_update >= update_interval:
                    if hasattr(self, 'progress_window') and self.progress_window:
                        self.root.after(0, lambda: self.progress_window.update(value, text))
            ...
            self.root.after(0, lambda: self.training_log.insert(
                tk.END, f"✅ Загружено {len(corpus)} предложений (все доступные)\n"
            ))
    ...
    def _show_corpus_stats(self, corpus):
        """Показать статистику корпуса - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        if not corpus:
            # Используем safe_config для избежания рекурсии
            self.safe_config(self.corpus_stats, text="Корпус пуст")
            return
        
        # Используем safe_config для избежания рекурсии
        self.safe_config(self.corpus_stats, text=f"Загружено {len(corpus)} параллельных предложений")
        self.show_page(0)
    
    def safe_config(self, widget, **kwargs):
        """Безопасное обновление виджетов для избежания рекурсии"""
        try:
            widget.config(**kwargs)
        except RecursionError:
            # Если возникает рекурсия, используем after для отложенного обновления
            self.root.after(10, lambda: widget.config(**kwargs))

    def show_page(self, page_num):
        """Показать указанную страницу корпуса"""
        if not self.current_corpus:
            return
        
        total_pages = (len(self.current_corpus) - 1) // self.page_size + 1
        page_num = max(0, min(page_num, total_pages - 1))
        self.current_page = page_num
        
        start_idx = page_num * self.page_size
        end_idx = min(start_idx + self.page_size, len(self.current_corpus))
        
        # Очищаем дерево
        for item in self.corpus_tree.get_children():
            self.corpus_tree.delete(item)
        
        # Добавляем записи для текущей страницы
        for i in range(start_idx, end_idx):
            pair = self.current_corpus[i]
            ...
            self.corpus_tree.insert('', 'end', values=(source_text, target_text))
    ...
    def previous_page(self):
        """Предыдущая страница"""
        if self.current_corpus and self.current_page > 0:
            self.show_page(self.current_page - 1)
    ...
    def change_page_size(self, event=None):
        ...
        try:
            ...
            if new_size != self.page_size:
                ...
                self.show_page(0)  # Перезагружаем с первой страницы
        except ValueError:
            pass
...
if __name__ == "__main__":
    ...
    print("   📄 Пагинация для просмотра всех записей Tatoeba")
